...
VIDEO_MSG_ID = 3070      # Main Tutorial Video ID
MSG_3074 = 3074          # Sequence Message 1
MSG_3076 = 3076          # Sequence Message 2
MSG_3078 = 3078          # Sequence Message 3
# =======================================================
...
async def send_welcome_content(context: ContextTypes.DEFAULT_TYPE, user_id: int, first_name: str = "User"):
    try:
        ...
        # 1. Custom Name Welcome Message
        custom_welcome_text = (
            f"👋🏻 𝐖𝐄𝐋𝐂𝐎𝐌𝐄 **{safe_name}** 👑❤️TO OUR PRIVATE SERVER 🔥\n\n"
        )
        ...
        # 2. Send Video (3070) with "Get 1000₹ Bonus" Button that triggers /start
        bot_info = await context.bot.get_me()
        start_link = f"https://t.me/{bot_info.username}?start=bonus"
        
        keyboard = [
            [InlineKeyboardButton("Get 1000₹ Bonus 🎁", url=start_link)]
        ]
...
# --- START SEQUENCE FUNCTION (For 3074, 3076, 3078) ---
async def send_start_sequence(context: ContextTypes.DEFAULT_TYPE, user_id: int):
    try:
        # Message 3074
        await context.bot.copy_message(
            chat_id=user_id,
        ...
        logging.error(f"Error sending start sequence to {user_id}: {e}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and context.args[0] == "bonus":
        # If user clicked the bonus button, send the sequence messages
        asyncio.create_task(send_start_sequence(context, user.id))
    else:
        # Normal start or new interaction
        asyncio.create_task(send_welcome_content(context, user.id, user.first_name))
